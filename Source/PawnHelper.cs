...
namespace VPEAutoCastBuffs
{
    ...
    {
        internal static IEnumerable<Pawn> GetPawnsWithHediff(
        ...
        internal static IEnumerable<Pawn> GetPawnsDown(IEnumerable<Pawn> pawns)
        {
            return pawns.Where(pawn => PawnIsDown(pawn));
        }
        ...
        internal static Pawn GetLowestSensitivity(IEnumerable<Pawn> pawns)
        {
        ...
        internal static bool PawnHasHediff(Pawn pawn, string hediffDefName)
        {
            return pawn?.health?.hediffSet?.hediffs.Any(hediff =>
                    hediff.def.defName == hediffDefName
                ) ?? false;
        }
        ...
        internal static IEnumerable<Pawn> GetPawnsWithDamagedEquipment(IEnumerable<Pawn> pawns)
        {
            ...
            return pawns.Where(colonist =>
                (
                    ...
                    && ThingHelper.ThingIsDamaged(colonist.equipment.Primary)
                )
                || (
                    colonist.apparel?.WornApparel.Any(apparel =>
                        ThingHelper.ThingIsDamaged(apparel)
