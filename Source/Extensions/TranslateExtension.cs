...
{
    public static Verse.TaggedString TranslateSafe(this string self, params Verse.NamedArgument[] args)
    {
        if (!Verse.Translator.CanTranslate(self))
        {
            BetterAutocastVPE.DebugError($"Untranslated key: {self}", Verse.GenString.GetHashCodeSafe(self));
        }
